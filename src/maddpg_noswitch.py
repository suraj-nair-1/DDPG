            ...
            for i in xrange(MAX_EPISODES):
                ...
                try:
                    ...
                    for j in xrange(MAX_EP_STEPS):
                        ...
                        np.savetxt(LOGPATH+'actions6_'+str(OFFENSE) + "_"+str(PLAYER)+'.txt', a.flatten())
                        ...
                        while True:
                            try:
                                if OFFENSE:
                                    other1 = np.loadtxt(LOGPATH+'actions6_'+str(OFFENSE) + "_"+str(OTHERPLAYER)+'.txt')
                                    other2 = np.loadtxt(LOGPATH+'actions6_0_3.txt')
                                else:
                                    other1 = np.loadtxt(LOGPATH+'actions6_1_'+str(1)+'.txt')
                                    other2 = np.loadtxt(LOGPATH+'actions6_1_'+str(2)+'.txt')
                        ...
                        if (replay_buffer.size() > MINIBATCH_SIZE) and (ITERATIONS % 10 == 0):
                            ...
                            if (ITERATIONS % 1000000) == 0:
                                    actor.model_save(LOGPATH + "models/target12_"+str(OFFENSE)+"_"+str(PLAYER)+"_"+str(ITERATIONS)+".tflearn", target=True)
                            # break
                        ITERATIONS += 1
                        ep_reward += r
                        ...
                        if terminal:
                            ...
                            f = open(LOGPATH +'logging/logs71_' + str(PLAYER) + '.txt', 'a')
